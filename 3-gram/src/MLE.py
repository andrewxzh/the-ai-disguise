...
class MLE():
    def __init__(self, word_df_path, pos=None):
        """
        ...
        if pos != None: 
            df=df[df['Word'].apply(lambda x: is_pos(x, pos))]
        word_df=df.copy()
    ...
    def optimized_log_likelihood(self,alpha, log_p_values, log_q_values):
        ...
        ll = np.mean(np.log((1 - alpha) * np.exp(log_p_values) + alpha * np.exp(log_q_values) + epsilon))
        return -ll
    ...
    def inference(self,inference_file_path):
        """
